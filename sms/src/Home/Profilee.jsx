...
const Profilee = () => {
  ...
  useEffect(() => {
    if (uid) {
      axios.post("http://localhost:3001/home", { uid })
        .then((res) => {
          if (res.data.user) {
    ...
    <div className="flex flex-col items-center justify-center">
      ...
      <div className="absolute top-[-35px] text-2xl font-bold left-2">PROFILE</div>
        ...
        >
          ...
          <div className="absolute top-[-20px] left-0 text-xs">Registration Number</div>
        </div>
        ...
        >
           {userData.find(item => item.key === label.toLowerCase())?.value}
          <div className="absolute top-[-20px] left-0 text-xs">{label}</div>
        </div>
      ))}
        </div>
      ...
      <div className="relative grid lg:grid-cols-5 md:grid-cols-4 gap-5 gap-y-12 sm:grid-cols-2">
        ...
        {personal.map((label,index)=>(
          <div className="border p-1 rounded-lg relative bg-gray-700">{userData.find(item=> item.key === label.toLowerCase())?.value}
          <div className=" absolute top-[-20px] left-0 text-xs">{label}</div>
        </div>
        ))}
